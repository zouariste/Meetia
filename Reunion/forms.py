...
class MeetingForm(forms.ModelForm):
    ...
    date=forms.DateField(widget=forms.DateInput(attrs={'type': 'Date'}),validators=[present_or_future_date])
    time = forms.TimeField(widget=forms.TimeInput(attrs={'type': 'time'}))
    class Meta:
        model=Reunion
        fields = ('date', 'time', 'place', 'rapporteur',)
...
class RecordForm(forms.ModelForm):
    audio_file=forms.FileField(widget=forms.FileInput(attrs={'class':'form-control-file','type':'file'}))
    class Meta:

...
def pos_tag_text(text):
    ...
    tagged_lower_text = [
        (word.lower(), penn_to_wn_tags(pos_tag)) for word, pos_tag in tagged_text
    ]
...
def textrank_text_summarizer(DOCUMENT, num_sentences=2, feature_type="frequency"):
    sentences = parse_document(DOCUMENT)
    norm_sentences = normalize_corpus(sentences, lemmatize=True)
...
def summarizer(DOCUMENT):
    ...
    return textrank_text_summarizer(DOCUMENT, num_sentences // 4, feature_type="tfidf")
...
def html2pdf(meeting, invitations, points):
   ...
   <br><hr><p><b>Present Collaborators:</b> """.format(
        meeting.id,
        meeting.date,
        meeting.time,
        meeting.place,
        meeting.dirigeant.first_name,
        meeting.dirigeant.last_name,
        meeting.rapporteur.first_name,
        meeting.rapporteur.last_name,
    )
    for invitation in invitations:
        if invitation.confirmation == "P":
            html += """{0} {1},""".format(
                invitation.collaborateur.first_name, invitation.collaborateur.last_name
            )
    ...
    html += """</p><br><hr><ul>"""
    for point in points:
        html += """<li><b>Point {0}:</b> {1}</li><p>{2}</p><br>""".format(
            point.ordre, point.titre, point.resume
        )
    html += """</ul>"""
    file = HTML2PDF()
    file.add_page()

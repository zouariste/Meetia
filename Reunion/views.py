...
def all_meetings(request):
    if hasattr(request.user, "employe_dirigeant_related"):
        ...
        a = request.user.employe_rapporteur_related.reunion_set.all().filter(
            soumise=True
        )
        paginator = Paginator(a, 4)
        page = request.GET.get("page")
        ...
        invitations = (
            request.user.employe_collaborateur_related.collaborateur_invitations.all()
        )
        meetings = []
...
def detail_meeting(request, meeting_id):
    ...
    if (
        hasattr(request.user, "employe_dirigeant_related")
        and meeting.dirigeant == request.user.employe_dirigeant_related
    ):
        if not meeting.soumise:
            ...
            try:
                ...
                context = {
                    ...
                    "collaborateurs_not_invited": collaborateurs,
                    "points": points,
                    "form": form,
                }
    ...
    elif request.user.isCollaborateur and meeting.meeting_invitations.all().filter(
        meeting=meeting, collaborateur=request.user.employe_collaborateur_related
    ):
        context = {
            "meeting": meeting,
            "all_invitations": invitations,
...
def all_invitations(request):
    if hasattr(request.user, "employe_dirigeant_related"):
        meetings = request.user.employe_dirigeant_related.reunion_set.all().filter(
            soumise=True
        )
        invitations = []
...
def detail_invitation(request, invitation_id):
    ...
    if (
        hasattr(request.user, "employe_dirigeant_related")
        and invitation.meeting.dirigeant == request.user.employe_dirigeant_related
        and invitation.meeting.soumise == True
    ):
        context = {
...
def delete_invitation(request, invitation_id):
    ...
    if (
        hasattr(request.user, "employe_dirigeant_related")
        and invitation.meeting.dirigeant == request.user.employe_dirigeant_related
        and not invitation.meeting.soumise
    ):
        invitation.delete()
...
def edit_point(request, point_id):
    ...
    if (
        hasattr(request.user, "employe_dirigeant_related")
        and point.reunion.dirigeant == request.user.employe_dirigeant_related
        and not point.reunion.soumise
    ):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect(
                    "/meeting/{0}/".format(str(point.reunion.id))
                )
        form = PointForm(instance=point)

        return render(
            request,
            "points/editPoint.html",
            {
                "form": form,
            },
        )
    elif (
        request.user.isRapporteur
        and point.reunion.rapporteur == request.user.employe_rapporteur_related
    ):
        if point.reunion.hasPV and point.reunion.pv.soumis == True:
            return HttpResponseRedirect(
                "/meeting/edit/{0}/".format(str(point.reunion.id))
            )
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                html2pdf(
                    point.reunion,
                    point.reunion.meeting_invitations.all(),
                    point.reunion.points.all(),
                )
...
def add_record_point(request, meeting_id, point_id):
    ...
    form = RecordForm()
    if (
        hasattr(request.user, "employe_rapporteur_related")
        and meeting.rapporteur == request.user.employe_rapporteur_related
    ):
        if request.method == "POST":
...
def changeRecord(request, record_id):
    ...
    if (
        request.user.isRapporteur
        and record.point.reunion.rapporteur == request.user.employe_rapporteur_related
    ):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                changedRecord.save()
                return HttpResponseRedirect(
                    "/meeting/edit/" + str(record.point.reunion.id) + "/"
                )
        form = RecordForm(
            instance=record,
        )
...
def pdfCreate(request, meeting_id):
    ...
    if (
        request.user.isRapporteur
        and reunion.rapporteur == request.user.employe_rapporteur_related
        and not reunion.hasPV
    ):
...
def submit_pv(request, meeting_id):
    ...
    if (
        request.user.isRapporteur
        and pv.reunion.rapporteur == request.user.employe_rapporteur_related
    ):
